#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# train_lstm.py  (no calibration; NUM_WORKERS=0; Validation-picked threshold is applied to Holdout/Test)
...
from model import build_lstm_single, build_lstm_mh  
from config import get_lstm_config_space
...
# ==== Grouping keys (for leakage-safe splits) ====
# Used to construct group IDs for StratifiedGroupKFold; dataset-specific where possible.
GROUP_KEYS_MAP = {
...
def _as_list_datasets(tag: str) -> List[str]:
    """Split a dataset tag like 'a+b+c' into ['a','b','c']."""
    return [t for t in str(tag).split("+") if t]

# ==== Auto variant discovery (flat: <mode>_<variant>.npz) ====
def discover_variants_for_mode(datasets: List[str], mode: str) -> List[str]:
    """Scan output/<dataset>/ for files '{mode}_<variant>.npz' and collect the variants."""
...
def list_variants_per_dataset(datasets: List[str], mode: str) -> Dict[str, List[str]]:
    """List variants per dataset (useful for informative error messages)."""
    pat = re.compile(rf"^{re.escape(mode)}_(.+)\.npz$")
...
def load_combo_multi(datasets: List[str], variant: str) -> Optional[Dict[str, Any]]:
    """Concatenate combo packs over datasets; ensure consistent (T,2)."""
    Xs, ys, Ms = [], [], []
    for ds in datasets:
...
def make_groups(meta: pd.DataFrame, dataset_tag: str) -> np.ndarray:
    """
    Build group IDs for StratifiedGroupKFold:
    - Prefer dataset-specific keys from GROUP_KEYS_MAP if available in meta.
    - Otherwise, use available columns from GROUP_PRIORITY (fallback).
    """
    m = meta.copy()
    ...
    if not fallback_keys_all:
        # Last resort: unique per-row groups (no grouping information).
...
def _assert_no_key_overlap(meta: pd.DataFrame, tr_idx: np.ndarray, va_idx: np.ndarray):
    """
    Ensure no group leakage between train/val (or train/test) within each dataset.
    """
    if "Dataset" not in meta.columns:
        raise RuntimeError("Missing 'Dataset' column in meta for overlap check.")
...
def choose_threshold(y_true: np.ndarray, probs: np.ndarray, mode: Literal["f1","youden"]="f1") -> float:
    ...
    - mode='f1': maximize F1 on a dense grid plus candidate probs.
    - mode='youden': maximize TPR - FPR (Youden's J statistic).
    """
...
def save_weights(arch: str, train_tag: str, variant: str, cfg_idx: int, seed: int,
    ...
    """Save model weights to results/weights/<arch>/<train>/<variant>/config_k[/modality]/..."""
...
def train_and_eval(train_pack: Dict[str,Any], train_tag: str, arch_name: str, variant: str, modality: Optional[str],
    ...
    # Collect per-fold external-test logs if provided
    ...
    for fold, (tr_idx, va_idx) in enumerate(sgkf.split(np.zeros(len(y)), y, groups), start=1):
        ...
        model = build_lstm_mh(cfg, input_shape, pos_weight=pos_w) if modality is None else build_lstm_single(cfg, input_shape, pos_weight=pos_w)
        ...
        # --- pick threshold on validation predictions only
        device = next(model.parameters()).device
        ...
        # free CUDA memory and clean PL logs per fold
    ...
    summary = {
        ...
        "cross": cross_tag,
        "trained_on_datasets": train_ds_list,
        "tested_on_datasets": train_ds_list if not external_tests else sorted(list({str(k) for k in external_tests.keys()})),
        "hyperparameters": {k: (v.tolist() if isinstance(v, np.ndarray) else (v.item() if hasattr(v,'item') else v))
                            for k,v in cfg.items() if not k.startswith("__")},
        "val_summary": {
...
def run_cross(datasets: List[str], variant: str, mode: str, cfg_idx: int, seed: int,
              ...
              thr_mode: Literal["f1","youden"]="f1"):
    """
    Run a single sampled configuration (cfg_idx) for the given mode/variant across regimes:
    - none: per-dataset internal holdout (carved from the same dataset)
    - pairwise: train on pairs, evaluate on remaining datasets
    - lodo: leave-one-dataset-out
    - pool: train on all, evaluate on each dataset
    - transfer: train on one, evaluate on the others
    """
    # Sample an LSTM config
    cs  = get_lstm_config_space(seed=seed)
    ...
    # Preload per-dataset packs for reuse
    ds_packs = {ds: (load_combo_for_dataset(ds, variant) if mode=="combo" else _npz_pack(DATA_ROOT, ds, mode, variant))
                for ds in datasets}

    # If no external datasets are available, carve an internal 20% test split
...
# ==== CLI / Main ====
def parse_args():
    """Command-line interface for cross-dataset LSTM training."""
    ap = argparse.ArgumentParser("LSTM Cross-Training (combo/ecg/eda) – no calibration, with holdout & JSON logs")
    ap.add_argument("--datasets", default="dataset1,dataset2,dataset3,dataset8", help="Comma-separated list of datasets")
    ap.add_argument("--modes", default="ecg", help="combo, ecg, eda or all")
    ap.add_argument("--variants", default="auto", help="Comma list (e.g., none,z_window) or 'auto'")
...
def main():
    ...
    # Detect/validate variants per mode
    ...
    for mode in modes:
        if args.variants == "auto":
            ...
            if not auto_vs:
                ...
                msg = [f"Auto-scan found NO variants for mode='{mode}'. Expected files like: output/<dataset>/{mode}_<variant>.npz"]
